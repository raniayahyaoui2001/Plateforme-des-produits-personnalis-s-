...
<html>
    <head>
        <title> Détails catégories </title>
        <meta charset="UTF-8"/>
...
<h3>  Détails catégories  </h3>
<h4>
<br>
<a href='insertc.php'>  <i class='fa fa-plus plus' style='font-size:24px'></i></a>
<br>
...
";
 // Récupérer les notes depuis la Bd
            $categorie=new categorie();
            $les_com=$categorie->editAll();
            $nbr_com=count($les_com);
            
            echo "<table class='table' id='tab'>";
            //<td> Categorie </td>
            echo "<thead> <tr> <td> Nom du catégorie </td> <td> Image </td> <td> </td> </tr> </thead> <tbody>";
            foreach ($les_com as $icom)
            {    
                echo "<tr> <td>". $icom[1]."</td><td> <img src='img/". $icom[2]."' class='image-c'></td>";
                echo "<td>
                    <a  href='modifc.php?dd=$icom[0]'> <i class='fa fa-edit' style='font-size:24px'></i> </a>
                    <a onclick='if(confirm('voulez vous ?'))' href='suppc.php?dd=$icom[0]'><i class='fa fa-trash-o' style='font-size:24px;color:red'></i></a>
                </td></tr>";
            }
